...
const FileExplorer = ({
  ...
  const filesInPath = (p = path, ignoreFileStructure) =>
    files // Files and folders in current path, excluding full path in names, sorted with folders first.
      .map((file) => {
        ...
        const splitPath = isFolder
          ? file.path.slice(0, -1).split("/")
          : file.path.split("/");
        return {
          ...file,
          isFolder,
      ...
      .filter((file) => {
        ...
        if (file.splitPath === p || file.name.includes(".bucket."))
          return false; // If it's the folder itself or is a hidden file
        if (!p.length && file.splitPath.length === 1) return true; // This is a root file in the root path
        return !!(
          file.splitPath.slice(0, -1).toString() === p.toString() && p.length
        ); // If the file is in the right path, return true
      })
      .sort((first, second) => {
  ...
  useEffect(() => {
    ...
    getFiles();
    didRefresh();
  }, [idToken, doRefresh, didRefresh, getFiles, state]);
  ...
  const renameFile = () => {
    ...
    api
      ...
      .catch(() =>
        toast.dark(
          `❗ Couldn't rename file. Make sure a file with the same name doesn't already exist.`
        )
      );
    setFileToRename({});
  };
  ...
  const moveFile = (moveToParent) => {
    ...
    api
      ...
      .catch(() =>
        toast.dark(
          `❗ Couldn't move file. Make sure a file with the same name doesn't already exist in that folder.`
        )
      );
    setFileToMove({});
  };
    ...
    return filesInPath(path, ignoringFileStructure).map((file) => (
      <FileCard
        ...
        onDelete={() => {
          ...
          if (
            file.isFolder &&
            filesInPath(file.path.split("/").slice(0, -1)).length
          )
            return toast.dark(
              "❌ You must delete all files from this folder first."
            );
          setDeletionState({
            ...deletionState,
            open: true,
        ...
        onClickItem={async () => {
          ...
          // The user is selecting a folder to move the file to
    ...
    <div>
      ...
      <div className="explorer-buttons">
        ...
        <Button
          ...
          color="orange"
          size="tiny"
          onClick={() => setIgnoringFileStructure(!ignoringFileStructure)}
        >
          <Icon
            name={ignoringFileStructure ? "checkmark box" : "square outline"}
          />
      ...
      <Breadcrumb>
        ...
        {ignoringFileStructure && (
          <Breadcrumb.Section>all files and folders</Breadcrumb.Section>
        )}
        {!ignoringFileStructure &&
          path.map((folderName, folderDepth) => (
            <span>
              <Breadcrumb.Section
                link
                active={path.length === folderDepth + 1}
      ...
      <div className="files">
        {state.loading && (
          <Message icon negative={state.loadingError}>
            <Icon
              name={state.loadingError ? "warning sign" : "circle notched"}
        ...
        {ignoringFileStructure && (
          <Message
            warning
            content="Files that contain the text .bucket. may store dashboard settings or other information, so be careful when deleting or renaming them."
          />
        )}
        {!filesInPath().length && !state.loading && !ignoringFileStructure && (
        ...
        <Modal.Content>
          ...
          {deletionState.error && (
            <p style={{ textAlign: "center", color: "red" }}>
              Something went wrong and we couldn't delete that file.
            </p>
          )}
        </Modal.Content>
        <Modal.Actions>
      ...
      {/*Rename Modal*/}
      <Modal
        open={!!fileToRename.path}
        onClose={() => setFileToRename({})}
        size="mini"
      >
        <Header icon>
          <Icon name="edit" />
      ...
      {/* File Move Popup*/}
      <Portal
        open={!!fileToMove.path}
        onClose={() => setFileToMove({})}
        closeOnDocumentClick={false}
      >
        <div className="file-move-portal" style={{ position: "fixed" }}>
          <Segment className="file-move-portal-segment">
            <p>
              {!fileMoveDestination.name
                ? `Select the folder you want to move this file into.`
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              ...
              />
              {!fileMoveDestination.path &&
                fileToMove.path &&
                fileToMove.path.length > 1 && (
                  <Button
                    style={{ display: "inline-block" }}
                    size="small"
