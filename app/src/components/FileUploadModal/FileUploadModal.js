...
const FileUploadModal = ({ open, closeModal, path, onSuccess }) => {
    ...
    for (const [i, file] of state.files.entries()) {
      try {
        ...
        const uploadPolicy = await api
          ...
          .catch((err) =>
            handleStepFail(err, `Unable to get upload policy for file ${i + 1}`)
          );
        ...
        const [uploadPromise, cancelFunc] = api.postFile(
          uploadPolicy,
          file,
          (p) => dispatch({ type: "setProgress", rawProgress: p })
        ); // Post file and set progress callback
        ...
        await api
          .setPublicOrPrivate(file.name, shouldBePublic)
          .catch((err) =>
            handleStepFail(
              err,
              `Unable to make file ${i + 1} ${
  ...
  const onFilesChange = (event) => {
    ...
    let fileArray = Array.from(event.target.files).map((file) => {
      const fileName =
        parentPath + (state.folderUpload ? file.webkitRelativePath : file.name); // The absolute destination path of file. webkitRelativePath is the relative path of the file on the user's FS.
    ...
    if (state.folderUpload) {
      ...
      for (const file of fileArray) {
        const fileParentFolder =
          file.name.split("/").slice(0, -1).join("/") + "/";
        console.log(fileParentFolder);
        if (!folderPaths.includes(fileParentFolder))
          folderPaths.push(fileParentFolder);
      }
  ...
  return (
    <div>
      <Modal open={open}>
        ...
        <Modal.Content>
          <p>
            You can select multiple files or a single folder to upload. If you
            upload a folder, file structure will be preserved. Files will be
            uploaded to {(path || []).join("/") + "/"}.
          </p>
          ...
          {state.status && state.uploading && (
            <Progress percent={state.totalProgress} color="violet" />
          )}
        </Modal.Content>
        <Modal.Actions>
          <Button
